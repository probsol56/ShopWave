...
public record GetStockQuery(Guid ShopId) : IRequest<List<StockLevelDto>>;

public class GetStockQueryHandler(IAppDbContext db)
    : IRequestHandler<GetStockQuery, List<StockLevelDto>>
{
    public async Task<List<StockLevelDto>> Handle(GetStockQuery request, CancellationToken cancellationToken)
    {
